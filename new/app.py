...
def editar_usuario(cpf):
    ...
    # 2. Prepara os dados para validação (Garante que só terão números)
    cpf_url_limpo = re.sub(r'\D', '', str(cpf))
    cpf_sessao_limpo = re.sub(r'\D', '', str(session.get("usuario_cpf", "")))
    eh_admin = session.get("cargo") == "admin"
    
    # 3. Lógica de Permissão: Barra se NÃO for admin e os CPFs não baterem
    ...
    # Busca o usuário (forçando a limpeza do CPF do JSON também para evitar erros de comparação)
